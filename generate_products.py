...
products = []
for idx, rp in enumerate(rawProducts):
    cat = next(c for c in categories if c['id'] == rp['category'])
    color = cat['color'].replace('#', '')
    thumbColors = categoryThumbnails[cat['id']]
    urlName = rp['name'].replace(' ', '+')
    image = f"https://placehold.co/600x400/{color}/white?text={urlName}"
    ...
    products.append({
        ...
        'shortDescription': f"High-quality {rp['name'].lower()} for children.",
        'description': f"This {rp['name'].lower()} provides endless fun and development. Built with durable materials for safety and longevity. Perfect for engaging young minds and active bodies.",
        'longDescription': f"The {rp['name']} is designed to spark imagination and encourage physical activity. It features sturdy construction with non-toxic finishes, ensuring a safe play environment. Easy to assemble and maintain, it is an excellent addition to any play area. Recommended by educators and parents alike for its developmental benefits.",
        'features': [
        ...
        'image': image,
        'imageUrl': image,
        'thumbnails': [f"https://placehold.co/200x200/{tc}/white?text={urlName}" for tc in thumbColors]
    })

fileContent = f"""export interface Category {{
  id: string;
  name: string;
  slug: string;
  icon: string;
...
with open('/Users/sarthak/Downloads/Funplay/src/data/products.ts', 'w') as f:
    f.write(fileContent)
